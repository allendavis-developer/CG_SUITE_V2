...
async function handleBridgeForward(message, sender) {
  ...
  // Audit mode intake: open one Web EPOS product edit page in the upload worker tab and read its
  // current price, title, and category levels so the app can reverse-map the CG category.
  ...
  // Audit mode finish: edit each existing Web EPOS product in place (price and/or category),
  // skipping items with no diff. Mirrors openWebEposProductCreateForUpload but hits the edit URL.
